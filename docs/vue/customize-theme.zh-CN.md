...
ui-vue-antd的组件结构及样式和Antd React完全一致，你可以参考Antd React的定制方式进行配置。
...
- 样式必须加载 less 格式。
  ...
  - 如果你是通过 `'ui-vue-antd/dist/antd.css'` 引入样式的，改为 `ui-vue-antd/dist/antd.less`。
...
建立一个单独的 `less` 文件如下，再引入这个文件。
   ...
   ```less
   @import "~ui-vue-antd/dist/antd.less";   // 引入官方提供的 less 样式入口文件
